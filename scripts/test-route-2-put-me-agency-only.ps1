# Script de Prueba: Ruta 2 PUT /users/me - Solo AGENCY
# Prueba la actualizacion de perfil solo con rol AGENCY

$ErrorActionPreference = "Continue"

Write-Host "Prueba: PUT /users/me (Solo AGENCY)" -ForegroundColor Cyan
...
Write-Host "[INIT] Autenticando como AGENCY..." -ForegroundColor Yellow
...
try {
    ...
    if ($token) {
        Write-Host "[OK] Token obtenido para AGENCY`n" -ForegroundColor Green
    } else {
...
try {
    ...
    Write-Host "Bio actual: $($currentProfile.data.bio)" -ForegroundColor Gray
    if ($currentProfile.data.profile.agencyName) {
        Write-Host "Nombre de agencia: $($currentProfile.data.profile.agencyName)" -ForegroundColor Gray
    }
    Write-Host ""
} catch {
...
$updateBody1 = @{
    bio = "Bio actualizada desde PowerShell - AGENCY - $(Get-Date -Format 'HH:mm:ss')"
}
...
$updateBody2 = @{
    ...
    bio = "Bio con nombre actualizado - AGENCY - $(Get-Date -Format 'HH:mm:ss')"
}
