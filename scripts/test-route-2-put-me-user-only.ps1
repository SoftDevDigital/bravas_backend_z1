# Script de Prueba: Ruta 2 PUT /users/me - Solo USER
# Prueba la actualizacion de perfil solo con rol USER

$ErrorActionPreference = "Continue"

Write-Host "Prueba: PUT /users/me (Solo USER)" -ForegroundColor Cyan
...
try {
    ...
    if ($token) {
        Write-Host "[OK] Token obtenido para USER`n" -ForegroundColor Green
    } else {
...
$updateBody1 = @{
    bio = "Bio actualizada desde PowerShell - USER - $(Get-Date -Format 'HH:mm:ss')"
}
...
$updateBody2 = @{
    ...
    bio = "Bio con nombre actualizado - $(Get-Date -Format 'HH:mm:ss')"
}
